...
function loadProfile() {
  try {
    ...
    return raw ? JSON.parse(raw) : { gender: "", age: "", health: [] };
  } catch (e) {
    return { gender: "", age: "", health: [] };
  }
}
...
function ProfileSheet({ existing, onClose, onSave }) {
  ...
  return (
    ...
    >
      <div
        onClick={(event) => event.stopPropagation()}
        style={{ background: TOKENS.bgRaised, borderTop: `1px solid ${TOKENS.rule}`, width: "100%", maxWidth: 430, padding: "22px 22px 30px", fontFamily: TOKENS.fontBody, color: TOKENS.fg }}
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22 }}>
          <div>
            <Eyebrow>RECOMMENDATION PROFILE</Eyebrow>
            <div style={{ fontFamily: TOKENS.fontDisplay, fontSize: 24, fontWeight: 700, marginTop: 4 }}>내 추천 기준</div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: TOKENS.fgDim, cursor: "pointer", fontSize: 22, lineHeight: 1, padding: 4 }} aria-label="닫기">×</button>
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 20 }}>
          {[['female', '여성'], ['male', '남성'], ['other', '직접 선택'], ['none', '선택 안 함']].map(([key, label]) => (
            <button
              key={key}
              onClick={() => setGender(key === "none" ? "" : key)}
              style={{ flex: 1, minHeight: 38, padding: "7px 4px", background: gender === (key === "none" ? "" : key) ? TOKENS.accent : "none", color: gender === (key === "none" ? "" : key) ? TOKENS.bg : TOKENS.fgMid, border: `1px solid ${gender === (key === "none" ? "" : key) ? TOKENS.accent : TOKENS.rule}`, fontFamily: TOKENS.fontDisplay, fontSize: 11.5, cursor: "pointer", whiteSpace: "nowrap" }}
            >
              {label}
            </button>
          ))}
        ...
        <Eyebrow style={{ marginBottom: 8 }}>나이</Eyebrow>
        <input
          type="number"
          min="1"
          max="120"
          ...
          placeholder="선택 입력"
          style={{ width: "100%", background: "none", border: "none", borderBottom: `1px solid ${TOKENS.rule}`, color: TOKENS.fg, padding: "8px 0", fontSize: 14, fontFamily: TOKENS.fontBody, marginBottom: 22 }}
        />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 10 }}>
          {healthOptions.map(([key, label]) => (
            <button
              ...
              style={{ textAlign: "left", minHeight: 38, padding: "8px 10px", background: health.includes(key) ? "rgba(212,255,80,0.12)" : "none", color: health.includes(key) ? TOKENS.fg : TOKENS.fgMid, border: `1px solid ${health.includes(key) ? TOKENS.accent : TOKENS.rule}`, fontFamily: TOKENS.fontBody, fontSize: 12, cursor: "pointer" }}
            >
              {health.includes(key) ? "✓ " : "□ "}{label}
            </button>
          ))}
        </div>
        <p style={{ margin: "0 0 22px", color: TOKENS.fgDim, fontSize: 11, lineHeight: 1.5 }}>의료 진단이나 치료를 대신하지 않으며, 옷차림과 활동 주의사항 참고로만 사용해요.</p>

        <button onClick={() => onSave({ gender, age, health })} style={{ width: "100%", background: TOKENS.accent, color: TOKENS.bg, border: "none", padding: "15px 0", fontFamily: TOKENS.fontDisplay, fontSize: 14, fontWeight: 700, letterSpacing: "0.06em", cursor: "pointer" }}>
          저장하기
        </button>
      </div>
    </div>
